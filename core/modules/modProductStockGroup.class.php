...
/**
 * 	\defgroup   productstockgroup     Module ProductStockGroup
 *  \brief      ProductStockGroup module descriptor.
 *
 *  \file       htdocs/productstockgroup/core/modules/modProductStockGroup.class.php
 *  \ingroup    productstockgroup
 *  \brief      Description and activation file for module ProductStockGroup
 */
include_once DOL_DOCUMENT_ROOT.'/core/modules/DolibarrModules.class.php';

/**
 *  Description and activation class for module ProductStockGroup
 */
class modProductStockGroup extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModuleProductStockGroupName' not found (ProductStockGroup is name of module).
		$this->name = preg_replace('/^mod/i', '', get_class($this));

		// Module description, used if translation string 'ModuleProductStockGroupDesc' not found (ProductStockGroup is name of module).
		$this->description = "ProductStockGroupDescription";
		...
		// Key used in llx_const table to save module status enabled/disabled (where PRODUCTSTOCKGROUP is value of property name of module in uppercase)
		...
		$this->module_parts = array(
			...
			'css' => array(
				//    '/productstockgroup/css/productstockgroup.css.php',
			),
			// Set this to relative path of js file if module must load a js on all pages
			'js' => array(
				//   '/productstockgroup/js/productstockgroup.js.php',
			),
		...
		// Example: this->dirs = array("/productstockgroup/temp","/productstockgroup/subdir");
		$this->dirs = array("/productstockgroup/temp");

		// Config pages. Put here list of php page, stored into productstockgroup/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@productstockgroup");
		...
		$this->langfiles = array("productstockgroup@productstockgroup");
		...
		//$this->automatic_activation = array('FR'=>'ProductStockGroupWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(1 => array('PRODUCTSTOCKGROUP_MYNEWCONST1', 'chaine', 'myvalue', 'This is a constant to add', 1),
		//                             2 => array('PRODUCTSTOCKGROUP_MYNEWCONST2', 'chaine', 'myvalue', 'This is another constant to add', 0, 'current', 1)
		// );
		$this->const = array();
		...
		if (!isset($conf->productstockgroup) || !isset($conf->productstockgroup->enabled)) {
			$conf->productstockgroup = new stdClass();
			$conf->productstockgroup->enabled = 0;
		}

		// Array to add new pages in new tabs
		$this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@productstockgroup:$user->rights->productstockgroup->read:/productstockgroup/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
		// $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@productstockgroup:$user->rights->othermodule->read:/productstockgroup/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		...
		$this->dictionaries=array(
			...
			'tabcond'=>array($conf->productstockgroup->enabled, $conf->productstockgroup->enabled, $conf->productstockgroup->enabled),
		...
		*/
		...
		// Boxes/Widgets
		// Add here list of php file(s) stored in productstockgroup/core/boxes that contains a class to show a widget.
		$this->boxes = array(
			//  0 => array(
			//      'file' => 'productstockgroupwidget1.php@productstockgroup',
			//      'note' => 'Widget provided by ProductStockGroup',
		...
		$this->cronjobs = array(
			...
			//      'unitfrequency' => 3600,
			//      'status' => 0,
			//      'test' => '$conf->productstockgroup->enabled',
			//      'priority' => 50,
			//  ),
		);
		// Example: $this->cronjobs=array(
		//    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->productstockgroup->enabled', 'priority'=>50),
		//    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'$conf->productstockgroup->enabled', 'priority'=>50)
		// );
	...
	public function init($options = '')
	{
		...
		//$result = $this->_load_tables('/install/mysql/', 'productstockgroup');
		$result = $this->_load_tables('/productstockgroup/sql/');
		if ($result < 0) {
		...
		//$result4=$extrafields->addExtraField('productstockgroup_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'productstockgroup@productstockgroup', '$conf->productstockgroup->enabled');
		//$result5=$extrafields->addExtraField('productstockgroup_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'productstockgroup@productstockgroup', '$conf->productstockgroup->enabled');
